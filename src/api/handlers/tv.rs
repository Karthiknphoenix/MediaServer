...
pub async fn refresh_series_metadata(
    ...
    // If we have a TMDB ID, fetch episode details for each season
    if let Some(tmdb_id) = meta.tmdb_id {
        if let Some(key) = media_service::get_tmdb_api_key(&pool).await? {
            let seasons = media_service::get_series_seasons(&pool, &series_name).await?;
            
            for season_num in seasons {
                if let Ok(episodes) = fetch_season_details(tmdb_id, season_num, &key).await {
                    for ep in episodes {
                        let still_url = ep.still_path.map(|p| format!("https://image.tmdb.org/t/p/w500{}", p));
